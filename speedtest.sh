...
speedtest () {
	dlspeed=$(curl --connect-timeout 8 http://$1/$fileName -w "%{speed_download}" -o /tmp/$fileName -s | sed "s/\,/\./g")
	echo `expr ${dlspeed%.*} / 131072` | sed "s/$/ Mbps/;s/^/\tDownload Speed\: /";
	ulspeed=$(curl --connect-timeout 8 -F "file=@/tmp/$fileName" http://$1/webtests/ul.php -w "%{speed_upload}" -s -o /dev/null | sed "s/\,/\./g")
	echo `expr ${ulspeed%.*} / 131072` | sed "s/$/ Mbps/;s/^/\tUpload speed\:   /"; echo;
}
...
echo "Testing North America locations" ; echo
...
echo "Testing EU locations" ; echo
